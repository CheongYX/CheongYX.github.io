...
export default function LayoutShell({ lang, isEyeCareMode, showGrid, children }) {
  ...
  return (
    <div className={`min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col lg:flex-row relative overflow-hidden transition-all duration-700 ${isEyeCareMode ? 'eye-care-active' : ''}`}>
      ...
      >
        <div 
          className={`absolute inset-0 z-0 transition-opacity duration-500 pointer-events-none ${showGrid ? 'opacity-[0.5]' : 'opacity-0'}`} 
          style={{ backgroundImage: `linear-gradient(to right, #cbd5e1 1px, transparent 1px), linear-gradient(to bottom, #cbd5e1 1px, transparent 1px)`, backgroundSize: '24px 24px', maskImage: 'linear-gradient(to bottom, transparent, black 10%, black 90%, transparent)' }}
        ></div>
